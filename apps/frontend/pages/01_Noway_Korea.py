...
def main():
    ...
    df = load_data(r"apps/data/NowayKR.parquet")

    selected_option = sidebar.create_sidebar(text="Analytics", options=["Mentality Check", "In game", "Out of game", "Funny", "Educational"])
...
def funny(df: pd.DataFrame):
    ...
    with st.expander("Winrate by deaths"):
        ...
        plot_option = st.radio("Select Plotting Option:", ["Only Deaths -> Winrate", "Deaths -> Occurrences", "Both"])

        # Original bar chart (percentage of wins for each number of deaths)
        grouped_data = (df.groupby("deaths")["win"].value_counts(normalize=True)[:, True] * 100)
        ...
        if plot_option == "Only Deaths -> Winrate":
            ...
            ax1.plot(grouped_data.index, grouped_data, color='blue', marker='o', label='Percentage of Wins', linestyle='-', linewidth=2)
            ...
            ax1.bar(death_counts.index, death_counts, color='green', alpha=0.7, label='Death Counts')
            ...
            # Line graph for the percentage of wins
            ax2.plot(grouped_data.index, grouped_data, color='blue', marker='o', label='Percentage of Wins', linestyle='-', linewidth=2)
